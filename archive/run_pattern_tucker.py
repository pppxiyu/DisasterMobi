...
# when run as `python archive/run_pattern_tucker.py` from the repo root.
...
from utils.pattern_analysis.matching import (
...
OUTPUT_PLOTS = os.path.join(OUTPUT_DIR, 'archive', 'tucker')
...
def main():
    ...
    for city, graphs in [('Baton Rouge', br_graphs), ('Fort Myers', fm_graphs)]:
        ...
        if actual_interval is not None and actual_interval != _expected_interval:
            raise ValueError(
                ...
                f"Rebuild the graph pkl (notebook_drafts/Cubique/main_20260627.ipynb) or update config.py."
            )
        if n % SLOT_PER_DAY != 0:
            raise ValueError(
    ...
    if RUN_SPATIAL_FEATURES:
        from utils.pattern_analysis.osm_poi_features import (
    ...
    # ── Save ─────────────────────────────────────────────────────────────────
    out_path = os.path.join(OUTPUT_DIR, 'archive', 'tucker_results.pkl')
