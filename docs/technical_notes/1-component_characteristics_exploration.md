...
|---|---|---|
| residential | `counthu × LANDUSE_RESIDENTIAL_WEIGHT` (=1.0) | housing units (proxy; no "residential employment" exists) |
...
**TF-IWF reweighting** (`LANDUSE_WEIGHTING='tf_iwf'` and exponent
`GLOBAL_IWF_SCALE = 1.52`, both set in `run_pattern_nmf.py`). Plain shares
label ~80–88% of each city's block groups "residential" (at the 0.4 threshold;
84% pooled over the five city-events) because housing mass is ubiquitous. The
fix mirrors TF-IDF from text mining (documents = block groups, words = the six
categories) and works in three steps:
...
2. **Rarity weight (IWF)** — a category that is everywhere (residential) gets
   ...
   large one. The mass statistics are pooled over all five city-events' block
   groups (a city that appears with two events contributes its block groups
   once per event), so the pipeline computes ONE weight vector per run —
   `_global_landuse_classification` in `run_pattern_nmf.py` calls
   `pooled_iwf_weights` in `utils/data_processing/fetch_sld_landuse.py` — and
   classifies every city with those same weights. The exponent β below is the
   constant `GLOBAL_IWF_SCALE = 1.52` in `run_pattern_nmf.py`; larger β pushes
   labels further toward rare categories:

$$
\text{rarity weight} \;=\;
\left[\;\ln\!\left(
\frac{\text{total mass of all categories, pooled over all five city-events}}
     {\text{this category's mass, in the same pooled block groups}}
\right)\right]^{\beta}
$$

   Because the pooled statistics span the whole city-event registry, the
   classification is transductive: adding or removing a city-event shifts the
   weight vector, and with it every block group's label, slightly and in every
   city at once.
...
**Unknown** when a block group has neither housing nor jobs (zero in all five
city-events currently). Hard labels (one category per block group) follow the
...
`utils/pattern_analysis/space_function.py` but not used.



## Part 2 — The Two Correlation Analyses

Both analyses correlate **per-component characteristics** across one
city-event's components (each component is one observation; n = 11 for BR_Ida,
11 for FM_Ian, 9 for WM_Dorian, 9 for WM_Isaias, and 10 for LC_Laura; the five
city-events are analysed **separately**, never pooled). The shared feature
table (one row per component) is built in memory at every run and consumed
directly by the plots; it is not written to disk.
...
- **The analysis metric — defined so HIGHER = WORSE.** Since 2026-07-12 the
  analyses carry a single resilience metric (`RES_COLS = ['cum_loss']` in
  `run_pattern_nmf.py`):

| Metric | Definition | Notes |
|---|---|---|
| `cum_loss` | `Σ_d (1 − r(d))` | Net cumulative deviation from baseline over the disaster window, in day-equivalents. **Unclipped: above-baseline surges cancel below-baseline drops**, which keeps the metric linear in `r` and additive across components (a baseline-share-weighted sum of the component values equals the total-curve value). Positive values are net loss; **negative values are net gain** (the component ran above its baseline on balance). |

- **Retired metrics (historical).** Four further metrics — `drop_depth`
  (`1 − min(r)`), `early_collapse` (`14 − argmin(r)`, how soon the curve
  bottomed out), `recovery_day` (`1 + (last day with r < 0.9)`, using the
  threshold parameter `recovery_threshold = 0.9` and right-censored at the
  window length 15 when the component never recovered) and `recovery_deficit`
  (`1 − mean(r over the last 3 disaster days)`) — were retired from the
  analyses on 2026-07-12: they appear in no figure or output CSV, and the
  correlation block of §2.4 reads only `cum_loss`. Their computation is
  deliberately retained in `resilience_features` in
  `utils/pattern_analysis/component_features.py`, which still returns all five
  columns (an owner note in its docstring marks them as kept long-term);
  restoring a metric means adding its name back to `RES_COLS`. The
  `recovery_day` censoring convention — storing the numeric value 15 rather
  than NaN so that the hardest-hit components stayed in the rank correlation —
  is likewise historical, since no analysis consumes that metric any more.
...
- **Spearman rank correlation** (`scipy.stats.spearmanr`), computed pair by
  ...
  returns NaN. Ties are handled with average ranks.
- **Significance stars**: two-sided p < 0.05 (\*) and p < 0.01 (\*\*),
  **uncorrected for multiple comparisons**. The resilience block is a single
  metric row: 1 × 16 = 16 cells per city-event (`cum_loss` against
  weekday_ratio, the twelve functional shares, mean_distance, and the two
  median-income columns), so about 0.8 single stars are expected by chance at
  the 5% level (16 × 0.05). Before 2026-07-12 the block carried five metric
  rows (80 cells, roughly 4 chance stars). Treat isolated \* as suggestive and
  look for coherent patterns (e.g. from/to of the same category agreeing) —
  \*\* is the stronger evidence tier. Rough significance bars for p < 0.05 at the current component
  counts: |ρ| ≳ 0.60 at n = 11, ≳ 0.63 at n = 10, and ≳ 0.67 at n = 9.
...
**通用（两种都要）——换 solver。** sklearn 的 NMF（现在 `solver='cd'`，[decomposition.py:78](utils/pattern_analysis/decomposition.py:78) / [:117](utils/pattern_analysis/decomposition.py:117)）只做纯重构 $\|X-W_tH\|$，塞不进任何 context 项（$\mathrm{tr}(HLH^\top)$ 或 $\|Y-RG\|$）。所以得离开它、自己写非负乘法更新 / 投影梯度 solver。
